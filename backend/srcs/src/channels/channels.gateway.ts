...
export class ChannelsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleConnection(socket: SocketWithUser) {
		...
		socket.on('disconnecting', () => {
			...
			this.logger.debug(`Client disconnecting with ${socket.rooms.size} joined rooms`);
		...
		this.logger.debug(`Client connected with ${socket.rooms.size} joined rooms`);
	...
	async handleDisconnect(socket: Socket) {
		...
		this.logger.debug(`Client disconnect with ${socket.rooms.size} joined rooms`);
	}

	@SubscribeMessage('message')
	async handleMessage(@ConnectedSocket() socket: Socket, @MessageBody() data: string) {
